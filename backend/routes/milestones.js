...
router.post('/projects/:projectId/milestones', requireRole('admin', 'member'), async (req, res) => {
  const { projectId } = req.params;
  const project = await db.prepare('SELECT * FROM projects WHERE id = ? AND deleted_at IS NULL').get(projectId);
  if (!project) return res.status(404).json({ error: 'Project not found' });
  ...
  const milestone = await getMilestone(result.lastInsertRowid);
  logActivity(req.user.id, 'milestone.created', 'milestone', milestone.id, milestone.name);
  res.status(201).json(milestone);
});

router.patch('/milestones/:id', requireRole('admin', 'member'), async (req, res) => {
  const { id } = req.params;
  ...
  if (updates.length === 0) return res.json(await getMilestone(id));

  updates.push("updated_at = datetime('now')");
  values.push(id);

  await db.prepare(`UPDATE milestones SET ${updates.join(', ')} WHERE id = ?`).run(...values);

  const updated = await getMilestone(id);
  logActivity(req.user.id, 'milestone.updated', 'milestone', updated.id, updated.name);
  res.json(updated);
});

router.delete('/milestones/:id', requireRole('admin', 'member'), async (req, res) => {
  const { id } = req.params;
  const milestone = await db.prepare('SELECT * FROM milestones WHERE id = ?').get(id);
  if (!milestone) return res.status(404).json({ error: 'Milestone not found' });

  const txn = db.transaction(async () => {
    await db.prepare('UPDATE tasks SET milestone_id = NULL, updated_at = datetime(\'now\') WHERE milestone_id = ?').run(id);
    await db.prepare('DELETE FROM milestones WHERE id = ?').run(id);
  });
  await txn();
