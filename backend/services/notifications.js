...
export async function notifyTaskAssigned(task, assigneeId) {
...
export async function notifyCommentAdded(task, commenterId, commentBody) {
  const recipients = new Set();

  if (task.assignee_id != null) recipients.add(task.assignee_id);
  if (task.reporter_id != null) recipients.add(task.reporter_id);

  const watchers = await db.prepare('SELECT user_id FROM task_watchers WHERE task_id = ?').all(task.id);
...
export async function notifyDependencyDone(depTask) {
